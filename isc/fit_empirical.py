...
def init_directory(fit_params):
	...
	import sys
	root=os.getcwd()
	if fit_params['dataset']=='broockman':
	...
	if fit_params['dataset']=='pew':
		addon='emp_'+'pew_'+fit_params['optimization']+'_'+\
			str(''.join(random.choice(string.ascii_uppercase+string.digits) for _ in range(6)))
	directory=''
	if sys.platform == "linux" or sys.platform == "linux2" or sys.platform == "darwin":
		directory=root+'/data/'+addon #linux or mac
	elif sys.platform == "win32":
		directory=root+'\\data\\'+addon #windows
