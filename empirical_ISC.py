...
def run(exp_params):
	...
	P={
		...
		'std_init_opinion':20,
		'mean_intolerance':np.round(rng.uniform(0.5,1.5),decimals=1),
		'mean_susceptibility':np.round(rng.uniform(0,1.5),decimals=1),
		'mean_conformity':np.round(rng.uniform(-0.5,1),decimals=1),
	...
	for issue in brookman_dict.iterkeys():
		...
		for t in np.arange(0,P['t_sim']/P['t_measure']):
			...
			if jsd_e > max_similarity[issue]['sim']:
				max_similarity[issue]={'sim':jsd_e,'time':t,'expressed':expressed,'empirical':empirical}
	...
	for key in max_similarity.iterkeys():
		if max_similarity[key]['sim']>exp_params[1]:
			...
			sns.distplot(max_similarity[key]['empirical'],bins=range(1,8,1),kde=False,ax=ax1,label='empirical')
			sns.distplot(max_similarity[key]['expressed'],bins=range(1,8,1),kde=False,ax=ax1,label='model'),
			plt.legend()
			ax1.set(xlim=(1,7))
