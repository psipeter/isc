...
def crossover(evo_pop): #uniform crossover method
	crossed_pop=[]
	shuffle(evo_pop)
	for i in range(len(evo_pop)/2):
		parent1=evo_pop[i][0] #front of the list forward
		parent2=evo_pop[-1-i][0] #back of the list backward
		child1_params=[]
		child2_params=[]
		for j in range(len(evo_pop[0][0])):
			if uniform(0,1) < 0.5:
				child1_params.append(parent1[j])
				child2_params.append(parent2[j])
			else:
				child1_params.append(parent2[j])
				child2_params.append(parent1[j])
		crossed_pop.append([child1_params,0])
		crossed_pop.append([child2_params,0])
	return crossed_pop
...
def run(P):
	...
	for t in np.arange(0,P['t_sim']/P['t_measure']):
		...
		if jsd_e > max_similarity['sim']:
			max_similarity={'sim':jsd_e,'time':t*P['t_measure'],
								'expressed':expressed,'empirical':empirical}

	if max_similarity['sim']>P['sim_threshold']:
		print 'JSD=%s, t=%s' %(max_similarity['sim'],max_similarity['time'])
		root=os.getcwd()
		...
		plt.legend()
		ax1.set(xlim=(1,7))
		figure1.savefig('issue=%s_JSD=%s_t=%s.png' %(P['issue'],max_similarity['sim'],max_similarity['time']))
		plt.close(figure1)
...
def main():
	...
	pool = Pool(nodes=len(evo_pop))

	for g in range(generations):
		print '\ngen %s mean F = %s' %(g,np.average([evo_pop[ind]['F'] for ind in evo_pop.iterkeys()]))
		...
		remade_evo_pop=remake(evo_pop,new_gen_list)
		evo_pop=mutate(remade_evo_pop,evo_pop,mutation_rate,mutation_amount,evo_rng)
